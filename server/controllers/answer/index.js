...
module.exports = {
  crossOverAnalysis: async function(req, res) {
    ...
    try {
      ...
      //得到公用的a c的选项内容
      var data_a = await _model.findAll(options, mapxacondition);//a-scontent
      var data_c = await _model.findAll(options, mapycondition);//c-scontent
      ...
      for (let j = 0; j < data_a.length; j++) {
        var conditiona = {
          attributes: ['selectid'],
          where: {
            scontent: data_a[j].scontent,
            paperid:req.body.paperid,
            questionid: x[0]
          },
          raw:true
        };
        var innera = await _model.findAll(options, conditiona);
        a.push(innera[0]);
      }//a-scontent对应的a-selectid
      for (let j = 0; j < data_c.length; j++) {
        var conditionc = {
          attributes: ['selectid'],
          where: {
            scontent: data_c[j].scontent,
            paperid:req.body.paperid,
            questionid: req.body.Y
          },
          raw:true
        };
        var innerc = await _model.findAll(options, conditionc);
        c.push(innerc[0]);
      }//同上
      //自变量只有一个
        if (x.length < 2) {
        ...
        for (let i = 0; i < data_c.length; i++) {
          ...
          var cip = await _model.findAll(answer, condition2);//得到选C问题c[i]选项的ip组
          ...
          for (let j = 0; j < a.length; j++) {
            ...
            var aip = await _model.findAll(answer, condition1);//得到选A问题a[j]选项的ip组
...
/*               console.log("Bip")
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (data[i].type == 2) {
          ...
          for (let j = 0; j < dataoption.length; j++) {
            ...
            var selectnum = await _model.findAndCountAll(
              answer,
              contentcondition
            );
  ...
  postAnswers: async function(req, res) {
    ...
    try {
      ...
      await _model.create(answersheet, asbody);
      ...
      for (let i = 0; i < qes.length; i++) {
        ...
        //选择
          ...
          ansbody.selectid = qes[i].selectid;
          ansbody.answer = "";
        }
        await _model.create(answer, ansbody);
      }
    } catch (e) {
      body.code = "02";
      body.result = e.message;
